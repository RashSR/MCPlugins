...
public class Snowballarena extends EZPlugin implements PluginListener{
    ...
    private final int TIME_TO_NEXT_GAME_IN_SECONDS = 3;
    private int mapIndex = 1;

    //crafting? besserer Map Randomizer? yannick und tobi mode?
        ...
        if(caller instanceof Player player){ 
            if(args.length == 1){
                ...
                if(args[1].equals("off")){
                    ...
                    game.getWaitTask().setTimeInSeconds(time);
                }
            }
        }
    }
    ...
    public void rightClickevent(BlockRightClickHook event){
        ...
        if(blockClicked.getType() == BlockType.WallSign && isInHub(locX, blockClicked.getY(), locZ)){
            if(locX == 36){
                ...
                Utils.BroadcastServerMessage(Game.pluginName, "Zum Spielstart bitte einen " + ChatFormat.GOLD + "Spieler" + ChatFormat.DARK_GREEN + " auswaehlen."); 
        }
    }
        ...
        if(game.hasStarted()){
            ...
            if(event.getDamageSource().getDamagetype() == DamageType.FALL && !game.hasFallDmg())
                ...
                if(ent instanceof Player player){
                    if(player.getHealth() <= event.getDamageDealt()){
                        event.setCanceled();
                        if(game.isPlayer1(player)){
                            displayWinMessage(game.getPlayer2());
                            game.endGame(game.getPlayer2());
                        }else{
                            displayWinMessage(game.getPlayer1());
    ...
    private void displayWinMessage(Player winner){
        String remainingHealth = ChatFormat.GOLD + Float.toString(winner.getHealth()/2);
        String serverMessage = ChatFormat.GOLD + winner.getName() + ChatFormat.DARK_GREEN + " hat mit " 
            + remainingHealth + ChatFormat.DARK_GREEN + " verbleibenden Herzen gewonnen.";
        Utils.BroadcastServerMessage(Game.pluginName, serverMessage);
    }
    ...
    public void createNewGame(Game oldGame, Player winner){
        ...
        String serverMessage = ChatFormat.RED + "Punktestand: " + ChatFormat.GOLD + 
            player1old.getName() + " " + ChatFormat.GRAY + p1wins + ChatFormat.DARK_GREEN + 
            ":" + ChatFormat.GRAY + p2wins + " " + ChatFormat.GOLD + player2old.getName();
        Utils.BroadcastServerMessage(Game.pluginName, serverMessage);
        ...
        if(p1wins+p2wins == 11){
            ...
            Utils.BroadcastServerMessage(Game.pluginName, "Der Spieler " + ChatFormat.GOLD + winner.getName() + ChatFormat.DARK_GREEN + " hat gewonnen!");
            return;
        }
        ...
        this.game = new Game(oldGame.getWorld(), maps.get(mapIndex), this); //zweites ist map
        ...
        game.getWaitTask().setTimeInSeconds(TIME_TO_NEXT_GAME_IN_SECONDS);
    }
    ...
    @HookHandler
    public void FoodLevelHookEvent(FoodLevelHook event){
        if(game != null){
            Player player = event.getPlayer();
